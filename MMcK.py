...
def decrease_rate(tempEvent, listOfEvents):
    for event in listOfEvents:
        ...
        if event.eventType == 'arrival':
            ...
            print(
                f'[Time: {str(time)}] Remaining {event.eventType} time in Server {str(event.server_label)}: {str(event.rate)}')
...
def init_queue(q, server_label):
    ...
    print(
        f'[Time: {str(time)}] Next {newEvent.eventType} in {str(newEvent.rate)}')
    ...
    while n > 0:
        ...
        while (temp.eventType == 'service'):
            if (q.empty()) & (servers[temp.server_label].empty()):
                print(
                    f'[Time: {str(time)}] Next customer have not arrived yet in Queue.')
