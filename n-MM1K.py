...
def decrease_rate(tempEvent, listOfEvents):
    for event in listOfEvents:
        ...
        print(
            f"[Time: {str(time)}] Remaining {event.eventType} time in Queue {event.queue_label}: {str(event.rate)}")
    return listOfEvents
...
def main():
    ...
    while n > 0:
        ...
        while (temp.eventType == 'service') & (queues[temp.queue_label].empty()) & (servers[temp.queue_label].empty()):
            print(
                f'[Time: {str(time)}] Next customer have not arrived yet in Queue {temp.queue_label}. \n')
            haventArrived = True
